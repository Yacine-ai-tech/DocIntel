...
const BASE_URL = "https://gateway.ysiddo-ai-projects.app/docintel";
...
const ENDPOINTS: Endpoint[] = [
  ...
  // ── Batch processing ─────────────────────────────────────────────────────
  {
    ...
    desc: "Submit multiple documents for background async processing. Returns immediately with a job_id; files are processed concurrently (bounded by BATCH_MAX_CONCURRENCY, default 8) with per-file error isolation — one bad file never aborts the batch.",
    auth: true,
    ...
    response: "{ \"job_id\": \"b3f1c9de-...\", \"total\": 12 }",
  },
  {
    method: "GET",
    path: "/batch/{job_id}",
  ...
  // ── Camera / mobile pairing ──────────────────────────────────────────────
  {
    ...
    desc: "Generate a pairing token (24h expiry) and a base64 QR code image encoding the mobile-capture URL, so a phone can join the session by scanning it.",
    auth: true,
  ...
  },
  {
    ...
    path: "/camera/upload",
    group: "Camera / mobile pairing",
    desc: "The paired mobile device uploads a captured photo. The token is validated (must be active, unexpired) and the upload is automatically routed through Route B (local/remote Ollama vision, \"vision_local\" legacy alias) with automatic fallback to Route C (OCR) on failure. 403 if the token is invalid or expired.",
    auth: true,
